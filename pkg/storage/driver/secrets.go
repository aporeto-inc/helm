...
// We read `SecretsPageSize` secrets at a time so as not to overwhelm the
...
func (secrets *Secrets) List(filter func(*rspb.Release) bool) ([]*rspb.Release, error) {
	...
	opts := metav1.ListOptions{LabelSelector: lsel.String(), Limit: int64(secrets.SecretsPageSize)}
	...
	for isContinue != "" {
		secrets.Log("list: fetched %d secrets, more to fetch..\n", secrets.SecretsPageSize)
		opts = metav1.ListOptions{LabelSelector: lsel.String(), Limit: int64(secrets.SecretsPageSize), Continue: isContinue}
